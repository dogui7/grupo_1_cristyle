window.onload=function(){
    let form = document.querySelector('#formEditProfile');
    ...
    function validateRegisterForm() {
      ...
      let { name,lastname,image,} = form.elements
    ...
    function validateName(name) {
      ...
      if (name.value.length == 0) {
        error.innerHTML = 'Debes completar el nombre del usuario';
        return false;
      }
  
      // Si el nombre no tiene al menos 5 caracteres
      if (name.value.length < 5){
        error.innerHTML = 'El nombre del usuario debe tener al menos 5 caracteres';
        name.focus();
        return false;
      } 
    ...
    function validateName(lastname) {
        // En la vista hay un elemento <p> al final de cada campo, donde se pondrán los errores. Lo guardamos en la variable error
        let error = document.getElementById('lastNameErrorJs');
        // Le ponemos la clase "error" por defecto
        error.classList.add('error');
    
        
        if (lastname.value.length == 0) {
          error.innerHTML = 'Debes completar el apellido del usuario';
          return false;
        }
    
        
        if (lastname.value.length < 5){
          error.innerHTML = 'El apellido del usuario debe tener al menos 5 caracteres';
          lastname.focus();
          return false;
        } 
    ...
    function validateImage(image) {
      ...
      if (image.value.length == 0) {
        error.innerHTML = 'Debes subir una imagen de usuario';
        return false;
      }
